 ...
 */
package com.prochnow.ttsnotifications.util;
...
public class MenuColorizer {
     ...
     * Sets a color filter on all menu icons, including the overflow button (if it exists)
     */
    public static void colorMenu(final Activity activity, final Menu menu, final int color, final int alpha) {
    ...
    private static ImageButton getOverflowMenu(final Activity activity, final ViewGroup... viewGroup) {
        ...
        for (int i = 0, l = group.getChildCount(); i < l; i++) {
            ...
            if (v instanceof ImageButton && v.getClass().getSimpleName().equals("OverflowMenuButton")) {
